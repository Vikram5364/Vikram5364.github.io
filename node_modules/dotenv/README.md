...
$ echo "console.log('Hello ' + process.env.HELLO)" > users.js

$ dotenvx run --env-file=.env.production -- node users.js
Hello production
> ^^
```
...
$ dotenvx run --env-file=.env.local --env-file=.env -- node users.js
Hello local
```
...
$ DOTENV_KEY='<dotenv_key_production>' dotenvx run -- node users.js
